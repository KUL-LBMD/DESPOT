...
class DESPOT_Builder:
    ...
    def ref_probs(self, prob, cond_prob):
        ...
        # (r, lat, lon) grid bins.
        mean_plr = np.mean(cond_prob, axis = (3, 4))  # (P, L+1, r)

        # Weighted sum over protein types -> P(l), shape (L+1,)
        mean_lr = np.sum(wp[:, np.newaxis, np.newaxis] * mean_plr, axis = 0) + 1e-12 # [l,r]
        print(f'Sum over P(l | r): {np.sum(mean_lr, axis = 0)}')
        return mean_lr[np.newaxis, :, :, np.newaxis, np.newaxis]
