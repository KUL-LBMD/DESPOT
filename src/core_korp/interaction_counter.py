...
def _convert_file(filename, database):
    ...
    lig_df = converter.convert_ligand(
...
class KORP_Counter:
    ...
    def __init__(self, database):
        ...
        self.bin_arr = np.zeros((len(self.prot_types_list), len(self.lig_types_list), len(self.r_bins), len(self.theta_bins), len(self.phi_bins)), dtype = np.uint32)
    ...
    def  _process_interaction_pair(self, prot_df, lig_df):
        ...
        lig_subset = lig_df[lig_df['lig_type'].isin(self.lig_types_list)]
        ...
        for i in range(lig_coords.shape[0]):
            ...
            l_type = lig_types[i]
            l_idx = self.lig_to_idx[l_type]

            for j in nearby_indices[i]:
                p_coord = prot_coords[j]
                p_type = prot_types[j]
                p_idx = self.prot_to_idx[p_type]
                v1 = v1_arr[j]
                v2 = v2_arr[j]
                v3 = v3_arr[j]
                ...
                if r_idx >= 0 and not np.isnan(v1[0]):

                    theta = np.degrees(np.arccos(int_norm @ v1)) # Range [0, 180]
