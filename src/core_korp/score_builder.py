...
class DESPOT_Builder:
    ...
    def inverse_boltzmann(self, cond_prob, ref_prob):
        """
        score[p,l,r,theta,phi] = log[P(l | p,r,theta,phi) / P(l)]
        ...
        out_path = (
            DATA_DIR / 'potentials'
            / f'despot_ca_scores_{self.database.lower()}.npz'
        )
        np.savez_compressed(out_path,
            scores = scores)
...
class DFIRE_Builder:
    ...
    def __init__(self, database):
        ...
        self.r_bins = np.arange(2.0, 15.1, 0.1)
        self.theta_bins = np.deg2rad(np.arange(0, 183.0, 3.0))
        self.phi_bins = np.deg2rad(np.arange(0, 363.0, 3.0))
    ...
    def ref_probs(self, rho):
        ...
        for i in range(volume_normalization.shape[0]):
            theta_i, theta_e = self.theta_bins[i], self.theta_bins[i+1]
            theta_mid = (theta_i + theta_e) / 2
            theta_factor = np.sin(theta_mid) * (theta_e - theta_i)
    ...
    def inverse_boltzmann(self, cond_prob, ref_prob):
        ...
        out_path = (
            DATA_DIR / 'potentials'
            / f'dfire_ca_scores_{self.database.lower()}.npz'
        )
        np.savez_compressed(out_path,
            scores = scores)
